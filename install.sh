...
# Проверяем и переносим скрипты кастомизации на законные места, если они в корне репы
if [ -f "$REPO_DIR/wallpaper-menu.sh" ]; then
    mkdir -p "$HOME/.config/scripts"
    cp "$REPO_DIR/wallpaper-menu.sh" "$HOME/.config/scripts/"
fi

# Накатываем права на исполнение для ВСЕХ скриптов автоматизации автоматического меню
if [ -d "$HOME/.config/scripts" ]; then
    chmod +x "$HOME/.config/scripts"/*.sh 2>/dev/null || true
fi
if [ -d "$HOME/linux_betterwindows/scripts" ]; then
    chmod +x "$HOME/linux_betterwindows/scripts"/*.sh 2>/dev/null || true
fi
